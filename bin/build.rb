...
def build_debian_package(build_number, service_path, service_name, package_name, docker=false)
  ...
  FileUtils.cp("#{PROJECT_ROOT}/configuration/docker/#{package_name}.yml", "deb/#{package_name}/#{package_name}.yml") if docker
  FileUtils.cp("#{PROJECT_ROOT}/configuration/#{package_name}.yml", "deb/ida/#{package_name}/#{package_name}.yml") unless docker

  package_file_name = debian_package_filepath(build_number, package_name)
  if docker
    puts "Building package for use in Docker"
...
opts = Trollop::options do
  ...
  opt :docker, 'Generate a package for use in a docker container'
end
...
puts('Task took: ' + Benchmark.realtime do
    ...
    service_map.each do |package_name, attributes|
      ...
      service_name = "ida-sample-rp"
      build_number = ENV.fetch('BUILD_NUMBER', '0')
      ensure_artefacts_directory_exists
      install_prefix = '/ida'
      remove_old_debian_package(package_name)
      build_debian_package(build_number, service_path, service_name, package_name, opts[:docker])
